...
class ConocimientoExperto:
    def __init__(self, cons):
        ...
        if cons.generacionInternaCE or cons.soloPuntajesCE:
            ...
            self.scores = self.correrFiltro(cons)
        else:
            print("ExpertKnowledge: Loading external EK.")
            self.scores = self.cargarCE(cons.EK_source, cons)
        
        #print self.scores
        self.ajustarPuntajes(cons) #Ensures that scores are non-negative.
    ...
    def correrFiltro(self, cons):
        ...
        fileName = str(cons.nombrearchivoSalidaCE) + '_'+str(cons.algoritmoFiltro) +'_scores.txt' 
        ...
        if not os.path.exists(fileName):
            ...
            if cons.algoritmoFiltro == "multisurf_turf":
                turf = TuRFMe(cons.env,cons.algoritmoFiltro, cons.turfPercent, dummy, dummy2)
                filterScores = turf.filterScores
                labelName = 'MultiSURF_TuRF'
                
            elif cons.algoritmoFiltro=="surfstar_turf":
                turf = TuRFMe(cons.env,cons.algoritmoFiltro, cons.turfPercent,cons.reliefSampleFraction, dummy)
                filterScores = turf.filterScores
                ...
                turf = TuRFMe(cons.env,cons.algoritmoFiltro, cons.turfPercent,cons.reliefSampleFraction,cons.reliefNeighbors)
                filterScores = turf.filterScores
                ...
                labelName = 'SURFStar'
            elif cons.algoritmoFiltro=="surf":
            ...
            try:
                orderedValueList = self.cargarCE(cons.outEKFileName + '_'+str(cons.algoritmoFiltro)+'_scores.txt', cons)
            except Exception as inst:
                print(type(inst))
                print(inst.args)
                print(inst)
                print('cannot open', cons.outEKFileName + '_'+str(cons.algoritmoFiltro)+'_scores.txt')
                raise 
            else:
                return orderedValueList 
    
    
    def cargarCE(self, pathName, cons):
