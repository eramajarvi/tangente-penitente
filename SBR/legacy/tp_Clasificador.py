"""
TANGENTE PENITENTE
Nombre: tp_Clasificador.py
Descripcion: Este módulo define un clasificador individual dentro 
             de la población de reglas, junto con todos los parámetros 
             respectivos. Se incluyen los métodos al nivel de 
             clasificador, incluyendo el emparejamiento de constructores 
             (covering, copia, reinicio), la subsunción, el cruce y la 
             mutación.  También se incluyen métodos de actualización de parámetros.
"""
...
class Clasificador:
    def __init__(self,a=None,b=None,c=None,d=None):
        ...
        if isinstance(c,list):
            ...
            self.copiaClasificador(a, b)
        elif isinstance(a,list) and b == None:
            self.reiniciarClasificador(a)
        else:
    ...
    #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------     
    def coveringClasificador(self, setSize, exploreIter, state, phenotype):
        ...
        if cons.useExpertKnowledge:  
            ...
            while len(self.specifiedAttList) < toSpecify:
                ...
                if state[target] != cons.labelMissingData: # If one of the randomly selected specified attributes turns out to be a missing data point, generalize instead.
                    ...
                    self.condition.append(self.construirCoincidencias(target, state)) 
                i += 1
        ...
        #-------------------------------------------------------
            ...
            for attRef in potentialSpec:
                if state[attRef] != cons.labelMissingData: # If one of the randomly selected specified attributes turns out to be a missing data point, generalize instead.
                    ...
                    self.condition.append(self.construirCoincidencias(attRef, state))

    def seleccAtributoRW(self, toSpecify):
    ...
    def copiaClasificador(self, clOld, exploreIter):
    ...
    def reiniciarClasificador(self, classifierList): 
    ...
    def match(self, state):
        ...
        for i in range(len(self.condition)):
            attributeInfo = cons.amb.formatData.attributeInfo[self.specifiedAttList[i]]
            #-------------------------------------------------------
    ...
    def cruzamientoUniforme(self, cl):
        ...
        if cons.env.formatData.discretePhenotype or random.random() < 0.5: #Always crossover condition if the phenotype is discrete (if continuous phenotype, half the time phenotype crossover is performed instead)
            ...
            for attRef in comboAttList:  
                attributeInfo = cons.amb.formatData.attributeInfo[attRef]
                #-------------------------------------------------------
    ...
    def arregloLimiteSpec(self,cl):
        ...
        if cons.doAttributeFeedback:
            ...
            toRemove = len(cl.specifiedAttList) - cons.amb.formatData.specLimit
    ...
    def mutacion(self, state, phenotype):
        ...
        if (len(self.specifiedAttList) + steps) >= cons.env.formatData.specLimit:
            highLim = cons.amb.formatData.specLimit
        else:
        ...
        if newRuleSpec == len(self.specifiedAttList) and random.random() < (1-cons.upsilon): #Pick one attribute to generalize and another to specify.  Keep overall rule specificity the same.
            ...
            if not cons.useExpertKnowledge or random.random() > pressureProb:
                ...
                genTarget = self.seleccGeneralRW(1)
                
            attributeInfo = cons.amb.formatData.attributeInfo[genTarget[0]]
            if not attributeInfo[0] or random.random() > 0.5: #GEN/SPEC OPTION
                ...
                self.mutarAtributosContinuos(useAT,genTarget[0])
            ...
            if len(self.specifiedAttList) >= len(state): #Catch for small datasets - if all attributes already specified at this point.
                ...
                if state[specTarget[0]] != cons.labelMissingData and (not useAT or random.random() < cons.AT.getTrackProb()[specTarget[0]]):
                    ...
                    self.condition.append(self.construirCoincidencias(specTarget[0], state)) #buildMatch handles both discrete and continuous attributes
                    changed = True
        ...
        #-------------------------------------------------------
            ...
            if not cons.useExpertKnowledge or random.random() > pressureProb:
                ...
                specTarget = self.seleccEspecificoRW(change)
            for j in specTarget:
                if state[j] != cons.labelMissingData and (not useAT or random.random() < cons.AT.getTrackProb()[j]):
                    #Specify Target
                    self.specifiedAttList.append(j)
                    self.condition.append(self.construirCoincidencias(j, state)) #buildMatch handles both discrete and continuous attributes
                    changed = True
        ...
        #-------------------------------------------------------
            ...
            if not cons.useExpertKnowledge or random.random() > pressureProb:
                ...
                genTarget = self.seleccGeneralRW(change)
            ...
            #-------------------------------------------------------
            for j in genTarget:
                attributeInfo = cons.amb.formatData.attributeInfo[j]
                if not attributeInfo[0] or random.random() > 0.5: #GEN/SPEC OPTION
                    ...
                    self.mutarAtributosContinuos(useAT,j)
    ...
    def seleccGeneralRW(self, count):
    ...
    def seleccEspecificoRW(self, count):
    ...
    def mutarAtributosContinuos(self, useAT, j):
        #-------------------------------------------------------
        ...
        if useAT:
            if random.random() < cons.AT.getTrackProb()[j]: #High AT probability leads to higher chance of mutation (Dives ExSTraCS to explore new continuous ranges for important attributes)
                ...
                attRange = float(cons.amb.formatData.attributeInfo[j][1][1]) - float(cons.amb.formatData.attributeInfo[j][1][0])
                i = self.specifiedAttList.index(j) #reference to the position of the attribute in the rule representation
    ...
    def chequeoRango(self):
        ...
        for attRef in self.specifiedAttList:
            if cons.env.formatData.attributeInfo[attRef][0]: #Attribute is Continuous
                trueMin = cons.amb.formatData.attributeInfo[attRef][1][0]
                trueMax = cons.amb.formatData.attributeInfo[attRef][1][1]
    ...
    # DELETION METHOD
    #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------      
    def obtenerProbEliminacion(self, meanFitness):
    ...
    # OTHER METHODS
    #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
    def construirCoincidencias(self, attRef, state):
    ...
    # PARAMETER UPDATES
    #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------     
    def actualizarEstadoEpoca(self, exploreIter):
    ...
    def actualizarTamanoConjuntoCoincidencias(self, matchSetSize): 
    ...
    def imprimirClasificador(self):
        ...
        #-------------------------------------------------------------------------------
        specificity = len(self.condition) / float(cons.amb.formatData.numAttributes)
        epoch = 0
