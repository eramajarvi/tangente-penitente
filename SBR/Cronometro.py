...
"""
La función de este módulo es en gran medida para fines de 
desarrollo y evaluación. Específicamente rastrea no sólo el 
tiempo de ejecución global de Tangente Penitente, sino que 
también rastrea el tiempo utilizado por los diferentes mecanismos 
clave del algoritmo. Este seguimiento probablemente desperdicia 
un poco de tiempo de ejecución, así que para un rendimiento 
...
class Cronometro:
    def __init__(self):
        """ Inicializa todos los valores de los cronometros para 
        el algoritmo """
    ...
    def detenerTiempoPRUEBA(self):
        ...
        self.PRUEBAGlobal += diff
    # ------------------------------------------------------------

    def devolverCronometroGlobal(self):
        """ Fija el cronometro final global. Llamado al final 
        de toda la ejecucion """

        self.tiempoGlobal = (time.time() - self.refInicioGlobal) + self.tiempoAgregado
    ...
    def cronometroPRUEBA(self):
        ...
        self.cronometroPRUEBAGlobal += 1

    def fijarReinicioCronometro(self, rehacerArchivo): 
        """ Fija todos los valores de los cronometros a los 
        escritos previamente en el perfil cargado """

        print(rehacerArchivo + "_EstadisticasPoblacion.txt")

        # Abre cada archivo de datos para leerlo
        try:
            objetoArchivo = open(rehacerArchivo+"_EstadisticasPoblacion.txt", 'rU')
        ...
        # tiempo previo global agregado con el reinicio
        listaTemp = lineaTemp.strip().split('\t')
        self.tiempoAgregado = float(listaTemp[1]) * 60
    ...
    def reportarTiempos(self):
        """ Reporta los resumenes de tiempo para esta ejecucion. 
        Devuelve un string listo para imprimirse """
